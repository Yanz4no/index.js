(() => {
  ...
  return {
    onLoad: () => {
      try {
        const v = window.vendetta || vendetta;
        const UserProfileStore = v.metro.findByProps("getUserProfile");
        const UserStore = v.metro.findByProps("getCurrentUser");
        ...
        if (UserProfileStore && UserStore) {
          unpatch = v.patcher.after("getUserProfile", UserProfileStore, (args, res) => {
